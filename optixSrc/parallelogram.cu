...
RT_PROGRAM void intersect_sphere(int primIdx){
    ...
    if( disc > 0.0f ) {
        ...
        if( rtPotentialIntersection( root1 ) ) {
            ...
            shading_normal = geometric_normal =make_float3(0.0 ,0.0, 1.0);// ((O + root1*ray.direction) - center) / radius;
        ...
        if( check_second ) {
            ...
            if( rtPotentialIntersection( root2 ) ) {
                ...
                shading_normal = geometric_normal = make_float3(0.0, 0.0, 1.0);//((O +root2*ray.direction) - center) / radius;
                rtReportIntersection( 0 );
            }
        }
    }
